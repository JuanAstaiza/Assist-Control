...
//PAGINACION
$totalRegistros= count(Cargo::getLista(null,null));
...
$lista = '';
$cargos = Cargo::getListaEnObjetos(null, $orden);
for ($i = 0; $i < count($cargos); $i++) {
    $cargo = $cargos[$i];
    $lista.= '<tr>';
    $lista.= "<td>{$cargo->getNombre()}</td>";
    $lista.= "<td>{$cargo->getPerfil()->getNombre()}</td>";
    $lista.= '<td>';
    $lista.= "<a href='principal.php?CONTENIDO=assistControl/cargosFormulario.php&codigo={$cargo->getCodigo()}' title='Modificar'><img src='presentacion/imagenes/AssitControl/modificar.png'/></a>";
    $lista.= "<img src='presentacion/imagenes/AssitControl/eliminar.png' title='Eliminar' onClick=eliminar('assistControl/cargosActualizar.php&codigo={$cargo->getCodigo()}'); />";
    $lista.= '</td>';
    $lista.= '</tr>';
}
?>
    <h3>LISTA DE CARGOS</h3><br/><br/>
    ...
    <table border="1">
        <tr><th>Cargo</th><th>Perfil</th><th><a href="principal.php?CONTENIDO=assistControl/cargosFormulario.php" title="Adicionar"><img src="presentacion/imagenes/AssitControl/adicionar.png"/></a></th></tr>
        <?=$lista?>
    </table>
