...
//PAGINACION
$totalRegistros= count(AreaEnsenanza::getLista(null,null));
...
$lista = '';
$areaEnsenanzas = AreaEnsenanza::getListaEnObjetos(null, $orden);
for ($i = 0; $i < count($areaEnsenanzas); $i++) {
    $areaEnsenanza = $areaEnsenanzas[$i];
    $lista.= '<tr>';
    $lista.= "<td>{$areaEnsenanza->getNombre()}</td>";
    $lista.= '<td>';
    $lista.= "<a href='principal.php?CONTENIDO=assistControl/areasEnsenanzaFormulario.php&codigo={$areaEnsenanza->getCodigo()}' title='Modificar'><img src='presentacion/imagenes/AssitControl/modificar.png'/></a>";
    $lista.= "<img src='presentacion/imagenes/AssitControl/eliminar.png' title='Eliminar' onClick=eliminar('assistControl/areasEnsenanzaActualizar.php&codigo={$areaEnsenanza->getCodigo()}'); />";
    $lista.= '</td>';
    $lista.= '</tr>';
}
?>
    <h3>LISTA DE AREA DE ENSE&Ntilde;ANZA</h3><br/><br/>
    ...
    <table border="1">
        <tr><th>Nombre</th><th><a href="principal.php?CONTENIDO=assistControl/areasEnsenanzaFormulario.php" title="Adicionar"><img src="presentacion/imagenes/AssitControl/adicionar.png"/></a></th></tr>
        <?=$lista?>
    </table>
