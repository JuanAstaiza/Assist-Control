...
for ($i = 0; $i < count($diseño_predeterminado); $i++) {
    ...
    $this->Cell(100, 8, '', 0);
    $this->Cell(100, 8, 'LISTADO DE HORAS EXTRAS', 0);
    // Salto de línea
    $this->Ln(20);
}
...
$pdf->Cell(30, 8,'HORA INICIO', 1);
$pdf->Cell(30, 8,'FECHA FIN', 1);
$pdf->Cell(30, 8,'HORA FIN', 1);
$pdf->Cell(30, 8,'SUBTOTAL', 1);
$pdf->Ln(8);
$filtrohorasextras=  'cedulaPersona='.$_GET['cedula'];
$horasextras = getListaHorasExtras($filtrohorasextras);
for ($i = 0; $i < count($horasextras); $i++) {
    $horaextra = $horasextras[$i];    
    $pdf->SetFont('Arial', '', 12);
    $pdf->Cell(60, 8, '', 0);
    $fechainicio=date("d-m-Y",strtotime($horaextra['fechainicio'])); 
    $pdf->Cell(30, 8, utf8_decode($fechainicio), 1);
    $pdf->Cell(30, 8, utf8_decode($horaextra['horainicio']), 1);
    $fechafin=date("d-m-Y",strtotime($horaextra['fechafin'])); 
    $pdf->Cell(30, 8, utf8_decode($fechafin), 1);
    $pdf->Cell(30, 8, utf8_decode($horaextra['horafin']), 1);
