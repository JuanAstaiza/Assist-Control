...
//informacion 1=Asistencia 2=Faltas
    ...
    if ($informacion==1) {
        ...
        if ($_GET['codCargo']!=null || $_GET['codPerfil']!=null) {
            ...
            $perfil=$_GET['codPerfil'];
            $filtroperfil='codigo='.$perfil;
        ...
        for ($i = 0; $i < count($registros); $i++) {
            ...
            $pdf->SetFont('Arial', '', 12);
            $pdf->Cell(9, 8,'', 0);
            $pdf->Cell(40, 8, utf8_decode($registro['cedulapersona']), 1);
            $filtro_persona=  'cedula ='.$registro['cedulapersona'];
            $empleado = getListaPersona($filtro_persona);
            for ($k = 0; $k < count($empleado); $k++) {
            $objeto = $empleado[$k];
            $pdf->Cell(45, 8, utf8_decode($objeto['primernombre'])." ". utf8_decode($objeto['primerapellido']), 1);
            }
