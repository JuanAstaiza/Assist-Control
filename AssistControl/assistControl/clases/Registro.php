...
class Registro {
   ...
   function getPersona(){
    ...
    public static function getLista($filtro, $orden) {
        ...
        $cadenaSQL = "select codigo, cedulaPersona,{$P}persona.primerNombre, {$P}persona.segundoNombre,{$P}persona.primerApellido, {$P}persona.segundoApellido, fecha, tipo from lemo_registro,lemo_persona where {$P}registro.cedulaPersona={$P}persona.cedula $filtro $orden;";
        return Conector::ejecutarQuery($cadenaSQL, $BD);
    }
    
 
    public static function getListaEnObjetos($filtro, $orden) {
        $datos = Registro::getLista($filtro, $orden);
	$registros=array();
        for ($i = 0; $i < count($datos); $i++) {
            $registros[$i] = new Registro($datos[$i], null);
        }
        return $registros;
    }
    ...
    public static function getListaEnObjetosReporte($filtro) {
        $datos = Registro::getListaReporte($filtro);
	$registros=array();
        for ($i = 0; $i < count($datos); $i++) {
            $registros[$i] = new Registro($datos[$i], null);
        }
