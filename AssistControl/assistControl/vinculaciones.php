...
//PAGINACION
$totalRegistros= count(Vinculacion::getLista(null,null));
...
$lista = '';
$vinculaciones = Vinculacion::getListaEnObjetos(null, $orden);
for ($i = 0; $i < count($vinculaciones); $i++) {
    $vinculacion = $vinculaciones[$i];
    $lista.= '<tr>';
    $lista.= "<td>{$vinculacion->getNombre()}</td>";
    $lista.= '<td>';
    $lista.= "<a href='principal.php?CONTENIDO=assistControl/vinculacionesFormulario.php&codigo={$vinculacion->getCodigo()}' title='Modificar'><img src='presentacion/imagenes/AssitControl/modificar.png'/></a>";
    $lista.= "<img src='presentacion/imagenes/AssitControl/eliminar.png' title='Eliminar' onClick=eliminar('assistControl/vinculacionesActualizar.php&codigo={$vinculacion->getCodigo()}'); />";
    $lista.= '</td>';
    $lista.= '</tr>';
}
?>
 <h3>LISTA DE VINCULACIONES</h3><br/><br/>
    ...
    <table border="1">
        <tr><th>Nombre</th><th><a href="principal.php?CONTENIDO=assistControl/vinculacionesFormulario.php" title="Adicionar"><img src="presentacion/imagenes/AssitControl/adicionar.png"/></a></th></tr>
        <?=$lista?>
    </table>
